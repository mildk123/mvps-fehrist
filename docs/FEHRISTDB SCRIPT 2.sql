...
CL_DESCRIPTION TEXT,
TASKID INT FOREIGN KEY REFERENCES TASKS (TASKID),
CREATED_BY varchar(50) CONSTRAINT df_CHECKLIST_CREATED_BY DEFAULT original_login(),
DATE_CREATED datetime CONSTRAINT df_CHECKLIST_DATE_CREATED DEFAULT getdate(),
UPDATE_BY varchar(50) CONSTRAINT df_CHECKLIST_UPDATE_BY DEFAULT original_login(),
DATE_UPDATED datetime CONSTRAINT df_CHECKLIST_DATE_UPDATED DEFAULT getdate()
)
GO
CREATE TRIGGER upd_CHECKLIST
ON dbo.CHECKLIST AFTER UPDATE
AS
BEGIN
    IF UPDATE(UPDATE_BY) AND UPDATE(DATE_UPDATED)
        RETURN
    ...
    FROM CHECKLIST
    JOIN Inserted
        ON CHECKLIST.CHECKID = CHECKLIST.CHECKID
END;
GO
